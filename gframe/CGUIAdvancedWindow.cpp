...
CGUIAdvancedWindow::CGUIAdvancedWindow(IGUIEnvironment* environment, IGUIElement* parent, s32 id, core::rect<s32> rectangle,
    const wchar_t *text, EWINDOW_CLOSE close, const core::position2di& minimized, ICursorControl* cursor, video::ITexture* texture)
...
MinimizedRectangle(minimized,core::position2di(minimized.X+rectangle.getWidth(),minimized.Y+Skin->getSize(EGDS_WINDOW_BUTTON_WIDTH)+5)),
BarRectangle(rectangle.UpperLeftCorner,core::position2di(rectangle.LowerRightCorner.X,rectangle.UpperLeftCorner.Y+Skin->getSize(EGDS_WINDOW_BUTTON_WIDTH)+5)),
...
IsNotifying(false), NotifyTimer(0), NotifyState(false), Texture(texture)
{
    ...
    ButtonRectangles = core::array<core::rect<s32> >(ButtonInfo.size());
    Buttons = core::array<IGUIButton*>(ButtonInfo.size());
...
//! Returns close behavior
EWINDOW_CLOSE CGUIAdvancedWindow::getCloseHandling() const
{
            ...
            case EMIE_LMOUSE_DOUBLE_CLICK:
                {
                    ...
                    else
                    {
                        ...
                        BarRectangle = core::rect<s32>(
                            NormalRectangle.UpperLeftCorner.X,
                            NormalRectangle.UpperLeftCorner.Y,
                            NormalRectangle.LowerRightCorner.X,
                            NormalRectangle.UpperLeftCorner.Y+Skin->getSize(EGDS_WINDOW_BUTTON_WIDTH)+5
                        );
...
irr::video::ITexture *CGUIAdvancedWindow::getImage()
{
	return Texture;
}
...
//! Writes attributes of the element (not TESTED)
void CGUIAdvancedWindow::serializeAttributes(io::IAttributes* out, io::SAttributeReadWriteOptions* options=0) const
{
    IGUIElement::serializeAttributes(out,options);
    out->addColor("CurrentIconColor",CurrentIconColor);
    out->addRect("ClientRect",ClientRect);
    out->addBool("IsDraggable",IsDraggable);
    out->addBool("DrawBackground",DrawBackground);
    out->addBool("DrawTitlebar",DrawTitlebar);
    out->addBool("IsActive",IsActive);
    out->addBool("IsMinimized",IsMinimized);
    out->addBool("IsBar",IsBar);
    out->addBool("IsMinimizable",IsMinimizable);
    out->addInt("CloseHandling",CloseHandling);
    out->addRect("NormalRectangle",NormalRectangle);
    out->addRect("MinimizedRectangle",MinimizedRectangle);
    out->addRect("BarRectangle",BarRectangle);
    out->addBool("UseGradient",UseGradient);
    out->addBool("IsNotifying",IsNotifying);
    out->addInt("NotifyTimer",NotifyTimer);
    out->addBool("NotifyState",NotifyState);
...
//! Reads attributes of the element (not TESTED)
void CGUIAdvancedWindow::deserializeAttributes(io::IAttributes* in, io::SAttributeReadWriteOptions* options=0)
{
    IGUIElement::deserializeAttributes(in,options);
    CurrentIconColor = in->getAttributeAsColor("CurrentIconColor");
    DragStart = core::position2d<s32>(0,0);
    Dragging = false;
    ClientRect = in->getAttributeAsRect("CurrentIconColor");
    ...
    IsActive = in->getAttributeAsBool("IsActive");
    IsMinimized = in->getAttributeAsBool("IsMinimized");
    IsBar = in->getAttributeAsBool("IsBar");
    CloseHandling = (EWINDOW_CLOSE)in->getAttributeAsInt("CloseHandling");
    NormalRectangle = in->getAttributeAsRect("NormalRectangle");
    MinimizedRectangle = in->getAttributeAsRect("MinimizedRectangle");
    BarRectangle = in->getAttributeAsRect("BarRectangle");
    UseGradient = in->getAttributeAsBool("UseGradient");
    NotifyTimer = 0;
    NotifyState = in->getAttributeAsBool("NotifyState");
