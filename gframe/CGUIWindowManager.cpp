		...
		case EET_GUI_EVENT:
		    {
                ...
                {
                    if (isChildWindow(event.GUIEvent.Caller))
                    {
                        CGUIAdvancedWindow* window = (CGUIAdvancedWindow*)event.GUIEvent.Caller;
                        if (window->getCloseHandling()==EWC_REMOVE)
                        {
                            bone* b = getBone( window );
							if (b)
							{
								b->dettach();
								core::list<bone*>::Iterator itb = find(Bones.begin(),Bones.end(),b);
								Bones.erase(itb);
							}
                            core::list<CGUIAdvancedWindow*>::Iterator itw = find(Windows.begin(),Windows.end(),window);
							Windows.erase(itw);
                            //window->remove();
                            return false; // let the managed window close by itself
                        }
                    }
                }
		    }
...
CGUIAdvancedWindow* CGUIWindowManager::addWindow( core::rect<s32> rectangle, const wchar_t *text, EWINDOW_CLOSE close, video::ITexture* texture)
{
    CGUIAdvancedWindow* aw = new CGUIAdvancedWindow( Environment, this, -1, rectangle, text, close, core::position2di(0,0), Cursor, texture);
...
void CGUIWindowManager::recalculateMinimizedWindow()
{
    ...
    {
    case EGWMMP_TOP:
        origin = core::position2di(MinimizedDirection==EGWMMD_FROM_TOP_LEFT?0:Parent->getAbsolutePosition().getWidth()-static_cast<s32>(MinimizedSize),0);
        move=core::position2di(MinimizedDirection==EGWMMD_FROM_TOP_LEFT?MinimizedSize:-static_cast<s32>(MinimizedSize),0);
        MinimizedBar = core::rect<s32>(0,0,Parent->getAbsolutePosition().getWidth(),21);
        break;
    case EGWMMP_BOTTOM:
        origin = core::position2di(MinimizedDirection==EGWMMD_FROM_TOP_LEFT?0:Parent->getAbsolutePosition().getWidth()-static_cast<s32>(MinimizedSize),Parent->getAbsolutePosition().getHeight()-21);
        move=core::position2di(MinimizedDirection==EGWMMD_FROM_TOP_LEFT?MinimizedSize:-static_cast<s32>(MinimizedSize),0);
        ...
        break;
    case EGWMMP_RIGHT:
        origin = core::position2di(Parent->getAbsolutePosition().getWidth()-static_cast<s32>(MinimizedSize),MinimizedDirection==EGWMMD_FROM_TOP_LEFT?0:Parent->getAbsolutePosition().getHeight()-21);
        move=core::position2di(0,MinimizedDirection==EGWMMD_FROM_TOP_LEFT?21:-21);
        MinimizedBar = core::rect<s32>(Parent->getAbsolutePosition().getWidth()-static_cast<s32>(MinimizedSize),0,Parent->getAbsolutePosition().getWidth(),Parent->getAbsolutePosition().getHeight());
        break;
    case EGWMMP_LEFT:
