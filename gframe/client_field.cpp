...
void ClientField::ShowSelectCard(bool buttonok, bool chain) {
	...
	for(size_t i = 0; i < ct; ++i) {
		...
		if(selectable_cards[i]->code)
			...
			mainGame->btnCardSelect[i]->setImage(imageManager.¿¨±³Í¼[selectable_cards[i]->controler + tagOffset]);
			mainGame->btnCardSelect[i]->setCover(imageManager.coverTop[selectable_cards[i]->controler + tagOffset]);
		}
	...
	for(size_t i = 0; i < ct; ++i) {
		...
		if(display_cards[i]->code) {
			...
			if(delay_swap && display_cards[i]->controler == swap_player)
				tagOffset = 2 - tagOffset;
			mainGame->btnCardDisplay[i]->setImage(imageManager.¿¨±³Í¼[display_cards[i]->controler + tagOffset]);
			mainGame->btnCardDisplay[i]->setCover(imageManager.coverTop[display_cards[i]->controler + tagOffset]);
		}
