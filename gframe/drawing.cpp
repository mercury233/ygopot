...
void Game::DrawCard(ClientCard* pcard) {
	...
	irr::video::ITexture* imgg=imageManager.卡背图[pcard->controler + tagOffset + locationOffset];
	matManager.mCard.setTexture(0,imgg );
	driver->setMaterial(matManager.mCard);
	driver->drawVertexPrimitiveList(matManager.vCardBack, 4, matManager.iRectangle, 2);

	//irr::video::ITexture* imgc=imageManager.coverTop[pcard->controler + tagOffset + locationOffset];
	//matManager.mCoverTop.setTexture(0, imgc);
	//driver->setMaterial(matManager.mCoverTop);
	//driver->drawVertexPrimitiveList(matManager.vCardBack, 4, matManager.iRectangle, 2);
...
void Game::DrawMisc() {
	...
	for(int i = 0; i < 5; ++i) {
		...
		if(pcard && (pcard->position & POS_FACEUP)) {
			int m = 803 - i * 68;
			const int up=235,down=272;
			adFont->draw(L"/", mainGame->Resize(m - 4, down, m + 4, down+20), 0xff000000, true, false, 0);
			adFont->draw(L"/", mainGame->Resize(m - 3, down+1, m + 5, down+21), 0xffffffff, true, false, 0);
			int w = adFont->getDimension(pcard->atkstring).Width;
			adFont->draw(pcard->atkstring, mainGame->Resize(m - 5 - w, down, m - 5, down+20), 0xff000000, false, false, 0);
			adFont->draw(pcard->atkstring, mainGame->Resize(m - 4 - w, down+1, m - 4, down+21),
			             pcard->attack > pcard->base_attack ? 0xffffff00 : pcard->attack < pcard->base_attack ? 0xffff2090 : 0xffffffff , false, false, 0);
			w = adFont->getDimension(pcard->defstring).Width;
			adFont->draw(pcard->defstring, mainGame->Resize(m + 4, down, m + 4 + w, down+20), 0xff000000, false, false, 0);
			adFont->draw(pcard->defstring, mainGame->Resize(m + 5, down+1, m + 5 + w, down+21),
			             pcard->defence > pcard->base_defence ? 0xffffff00 : pcard->defence < pcard->base_defence ? 0xffff2090 : 0xffffffff , false, false, 0);
			adFont->draw(pcard->lvstring, mainGame->Resize(779 - i * 71, up, 800 - i * 71, up+20), 0xff000000, false, false, 0);
			adFont->draw(pcard->lvstring, mainGame->Resize(780 - i * 71, up+1, 800 - i * 71, up+21),
			             (pcard->type & TYPE_XYZ) ? 0xffff80ff : (pcard->type & TYPE_TUNER) ? 0xffffff00 : 0xffffffff, false, false, 0);
		}
	}
...
void Game::setSW(unsigned int code, unsigned int fadeIn, unsigned int stay, unsigned int fadeOut)
{
	static Mutex mutex;
	mutex.Lock();
	if(!SWPlaying)
...
void Game::DrawSW()
{
	...
	irr::video::ITexture* sw = nullptr;

	bool succ = false;

	if(SWChanged)
	{
		auto it = summonWords.find(nSWID);
		if(it == summonWords.end())
		{
			auto it = summonWords.find(nSWID);
			if(it == summonWords.end())
			{
				std::wstringstream ss;
				ss << L"textures/summonwords/" << nSWID << L".png";
				std::wifstream in(ss.str());
				if (in.good())
				{
					auto sw = driver->getTexture(ss.str().c_str());
					if (sw)
					{
						summonWords[nSWID] = sw;
						succ = true;
					}
					else
					{
						summonWords[nSWID] = nullptr;
					}
				}
			}
		}
	...
	if(succ)
	{
		...
		stayTime = nStay;
		fadeOutAll = nFadeOut;
		fadeOutTime = nFadeOut;
	}


	auto it = summonWords.find(SWID);
	if(it == summonWords.end())
	{
		auto it = summonWords.find(SWID);
		if(it == summonWords.end())
		{
			std::wstringstream ss;
			ss << L"textures/summonwords/" << SWID << L".png";
			std::wifstream in(ss.str());
			if (in.good())
			{
				auto sw = driver->getTexture(ss.str().c_str());
	...
	const unsigned int xOff = 220, yOff = 35;
	...
	if(fadeInTime > 0)
	{
		...
		auto size = sw->getSize();
		unsigned int xRight = xOff + size.Width * (fadeInAll - fadeInTime) / fadeInAll, yDown = yOff + 125;
		core::recti clip(0, 0, xRight, yDown);
		driver->draw2DImage(sw, mainGame->Resize(xOff, yOff), core::recti(0, 0, size.Width, size.Height), &clip,
	...
	{
		auto size = sw->getSize();
		driver->draw2DImage(sw, mainGame->Resize(xOff, yOff), core::recti(0, 0, size.Width, size.Height), 0,
			SColor(255, 255, 255, 255), true);
		stayTime--;
		return;
	}
	...
	if(fadeOutTime > 0)
	{
		...
		auto size = sw->getSize();
		driver->draw2DImage(sw, mainGame->Resize(xOff, yOff), core::recti(0, 0, size.Width, size.Height),
			0, SColor(255 * fadeOutTime / fadeOutAll, 255, 255, 255), true);
		fadeOutTime--;
...
void Game::DrawSpec() {
	if(showcard) {
		//change
		irr::video::ITexture* 特写卡图=imageManager.GetTexture(showcardcode);
		rect<s32> 原始大小=recti(vector2d<s32>(0,0),特写卡图->getOriginalSize());
		switch(showcard) {
		case 1: {
			//driver->draw2DImage(imageManager.GetTexture(showcardcode), mainGame->Resize(574, 150));
			driver->draw2DImage(imageManager.GetTexture(showcardcode),  mainGame->Resize(574, 154, 751, 404),原始大小);
			driver->draw2DImage(imageManager.tMask, mainGame->Resize(574, 150, 574 + (showcarddif > 177 ? 177 : showcarddif), 404),
		...
		case 2: {
			//driver->draw2DImage(imageManager.GetTexture(showcardcode), mainGame->Resize(574, 150));
			driver->draw2DImage(imageManager.GetTexture(showcardcode),  mainGame->Resize(574, 154, 751, 404),原始大小);
			driver->draw2DImage(imageManager.tMask, mainGame->Resize(574 + showcarddif, 150, 761, 404), recti(0, 0, 177 - showcarddif, 254), 0, 0, true);
			showcarddif += 15;
			if(showcarddif >= 177) {
				showcard = 0;
			}
		...
		case 3: {
			//driver->draw2DImage(imageManager.GetTexture(showcardcode), mainGame->Resize(574, 150));
			driver->draw2DImage(imageManager.GetTexture(showcardcode),  mainGame->Resize(574, 154, 751, 404),原始大小);
			driver->draw2DImage(imageManager.tNegated, mainGame->Resize(536 + showcarddif, 141 + showcarddif, 793 - showcarddif, 397 - showcarddif), recti(0, 0, 128, 128), 0, 0, true);
			if(showcarddif < 64)
				showcarddif += 4;
			break;
		}
		case 4: {
			...
			//change
			driver->draw2DImage(imageManager.GetTexture(showcardcode), mainGame->Resize(574, 154, 751, 404),
			                    原始大小, 0, matManager.c2d, true);
			if(showcarddif < 255)
				showcarddif += 17;
			break;
		}
		case 5: {
			...
			driver->draw2DImage(imageManager.GetTexture(showcardcode), mainGame->Resize(662 - showcarddif * 0.69685f, 277 - showcarddif, 662 + showcarddif * 0.69685f, 277 + showcarddif),
			                    原始大小, 0, matManager.c2d, true);
			if(showcarddif < 127)
				showcarddif += 9;
			break;
		...
		case 6: {
			//driver->draw2DImage(imageManager.GetTexture(showcardcode), mainGame->Resize(574, 150));
			driver->draw2DImage(imageManager.GetTexture(showcardcode),  mainGame->Resize(574, 154, 751, 404),原始大小);
			driver->draw2DImage(imageManager.tNumber, mainGame->Resize(536 + showcarddif, 141 + showcarddif, 793 - showcarddif, 397 - showcarddif),
		...
		case 7: {
			...
			irr::gui::Draw2DImageQuad(driver, imageManager.GetTexture(showcardcode), 原始大小, corner);
			showcardp++;
			showcarddif += 9;
			if(showcarddif >= 90)
				showcarddif = 90;
		...
		case 101: {
			...
			if(showcardp < 10) {
				...
				if(dInfo.vic_string && (showcardcode == 1 || showcardcode == 2)) {
					driver->draw2DRectangle(0xa0000000, mainGame->Resize(540, 320, 800, 340));
					guiFont->draw(dInfo.vic_string, mainGame->Resize(502, 321, 840, 340), 0xff000000, true, true);
					guiFont->draw(dInfo.vic_string, mainGame->Resize(500, 320, 840, 340), 0xffffffff, true, true);
				}
...
void Game::ShowElement(irr::gui::IGUIElement * win, int autoframe) {
	...
	for(auto fit = fadingList.begin(); fit != fadingList.end(); ++fit)
		if(win == fit->guiFading)
		{
			...
			fit=fadingList.erase(fit);
		}
	//new
	if(exit_on_return && win == mainGame->wMainMenu)
		mainGame->device->closeDevice();
	//new
	if(win == mainGame->wLanWindow){
	...
	if(win == mainGame->wLanWindow){
		//imageManager.tBackGround = driver->getTexture(DataManager::随机取档(L"textures",L"bg.jpg"));
...
void Game::HideElement(irr::gui::IGUIElement * win, bool set_action) {
	...
	//new
	if ( !win->isVisible() ){
		win->setRelativePosition(fu.fadingSize);
		return;
	}
	fu.fadingDiff.X = fu.fadingSize.getWidth() / 10;
	fu.fadingDiff.Y = (fu.fadingSize.getHeight() - 4) / 10;
...
void Game::DrawDeckBd() {
	//new
	if(!mainGame->deckBuilder.is_draging && ( !deckManager.histroy_deck.size() || deckManager.histroy_deck.rbegin()->main.size() != deckManager.current_deck.main.size() || deckManager.histroy_deck.rbegin()->extra.size() != deckManager.current_deck.extra.size()  ||deckManager.histroy_deck.rbegin()->side.size() != deckManager.current_deck.side.size() )){
		Deck current = deckManager.current_deck;
		deckManager.histroy_deck.push_back(current);
	}
	...
	if(deckBuilder.is_draging) {
		DrawThumb(deckBuilder.draging_pointer, position2di(deckBuilder.dragx - 22 * mainGame->window_size.Width / 1024, deckBuilder.dragy - 32 * mainGame->window_size.Width / 1024), deckBuilder.filterList, true);
	}
}
}
