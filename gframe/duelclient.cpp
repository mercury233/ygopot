...
int waitfortcp(void* stat)
{
	unsigned int *state=(unsigned int*)stat;
...
std::deque<HostPacket> DuelClient::hosts;
std::set<unsigned int> DuelClient::remotes;
event* DuelClient::resp_event = 0;

//new
event_base* DuelClient::client_base_s = 0;
bufferevent* DuelClient::client_bev_s = 0;
unsigned char DuelClient::connect_state_s = 0;
unsigned short DuelClient::gameport_s = 0;
unsigned char DuelClient::create_game_s = 0;
bool DuelClient::on_local = false;
std::vector<std::pair<std::wstring, std::wstring> > DuelClient::otherServers;
std::vector<std::pair<std::wstring, std::wstring> > DuelClient::stickServers;

//change
bool DuelClient::StartClient(unsigned int ip, unsigned short port, unsigned char create_game) {
	if(connect_state)
		return false;
	create_game_s=create_game;
	if(ip==htonl(SERVER_IP) && (port >9999 || port == SERVER_PORT) ){
		gameport_s=port;
		return StartGame_s();
	}
	...
	bufferevent_setcb(client_bev, ClientRead, NULL, ClientEvent,(void*)client_base);
	...
	if(create_game_s < 2 ) {
		...
		*/
		Thread::NewThread(ConnectTimeout, (void*)50);
	}
...
//new
bool DuelClient::StartGame_s() {
	if(connect_state_s !=2)
		return false;
	connect_state = 0x1;
	connect_state_s=3;
	client_base=client_base_s;
	client_bev=client_bev_s;
	rnd.reset(time(0));
	if(create_game_s < 2 ) {
		Thread::NewThread(ConnectTimeout,(void*)100);
	}
	mainGame->btnCreateHost->setEnabled(false);
	mainGame->btnJoinHost->setEnabled(false);
	mainGame->btnJoinCancel->setEnabled(false);
	mainGame->btnHostConfirm->setEnabled(false);
	mainGame->btnHostCancel->setEnabled(false);
	ClientEvent(client_bev_s,BEV_EVENT_CONNECTED,(void*)client_base_s);
	return true;
}

//new
int DuelClient::StartClient_s(void* v0){
	sockaddr_in sin;
	memset(&sin, 0, sizeof(sin));
	sin.sin_family = AF_INET;
	...
	while(!Thread::退出进程){
		if(!on_local)
		{
			client_base_s = event_base_new();
			if(!client_base_s)
				continue;
			client_bev_s = bufferevent_socket_new(client_base_s, -1, BEV_OPT_CLOSE_ON_FREE);
			bufferevent_setcb(client_bev_s, ClientRead, NULL, ClientEvent_s,(void*)client_base_s);
			client_base=client_base_s;
			client_bev=client_bev_s;
			if (bufferevent_socket_connect(client_bev_s, (sockaddr*)&sin, sizeof(sin)) >= 0) {
				connect_state_s=1;
				event_base_dispatch(client_base_s);
				bufferevent_free(client_bev_s);
				event_base_free(client_base_s);
				if(! on_local)
					connect_state = 0;
				if(connect_state_s==255)
				{
					mainGame->wLanWindow->setText(dataManager.GetIntraStrings(7));
					Thread::进程数--;
					return 1;
				}
				connect_state_s = 0;
...
//change
int DuelClient::ConnectTimeout(void* ctx) {
	int tim=(int)ctx;
	while(--tim>-1)
	{
		if(connect_state == 0 || connect_state == 0x7 || connect_state_s==4)
		{
			Thread::进程数--;
			return 0;
		}
...
{	
	if(!is_closing) {
		...
		if(create_game_s != 1)
			...
			mainGame->env->addMessageBox(L"", dataManager.GetIntraStrings(3));
		mainGame->gMutex.Unlock();
	}
}

void DuelClient::StopClient(bool is_exiting) {
	if((!connect_state_s || connect_state_s==255) && !on_local)
		return;
	if(connect_state != 0x7 && !is_exiting)
		return;
	is_closing = is_exiting;
	//change
	if(on_local || is_exiting)
		event_base_loopbreak(client_base);
	else 
		ClientThread((void*)2);
	if(is_qre_recording)
...
static void play(int code, const char* prefix)
{
	std::stringstream ss;
	ss << "movies\\" << prefix << code << ".mp4";
	std::string filename(ss.str());
	std::ifstream file(filename);
	if(file.good())
		PlayMovie((char*)filename.data());
}

void DuelClient::summonwords(int code)
{
	auto it = dataManager.summonwords.find(code);
	if (it == dataManager.summonwords.end())
	{
		return;
	}
	...
	int start = 0;
	auto sep = str.find('\\');
	while (sep != std::wstring::npos)
	{
		mainGame->AddChatMsg((wchar_t*)(str.substr(start, sep - start).c_str()), 8);
		start = sep + 1;
		sep = str.find('\\', start);
	}
	mainGame->AddChatMsg((wchar_t*)(str.substr(start).c_str()), 8);
}
...
void DuelClient::ClientEvent(bufferevent *bev, short events, void *ctx) {
	if (events & BEV_EVENT_CONNECTED) {
		...
		//change
			event_base_loopexit((event_base*)ctx, 0);
	}
}

//new
void DuelClient::ClientEvent_s(bufferevent *bev, short events, void *ctx) {
	if (events & BEV_EVENT_CONNECTED) {
		CTOS_PlayerInfo cspi;
		BufferIO::CopyWStr(mainGame->ebNickName->getText(), cspi.name, 20);
		SendPacketToServer(CTOS_PLAYER_INFO, cspi);
		bufferevent_enable(bev, EV_READ);
		connect_state_s=2;
		mainGame->wLanWindow->setText(dataManager.GetIntraStrings(5));
		...
		event_call.GUIEvent.Caller = ygo::mainGame->btnLanRefresh;
		mainGame->device->postEventFromUser(event_call);
		}
	} else if (events & (BEV_EVENT_EOF | BEV_EVENT_ERROR)) {
		if(Thread::退出进程)
			return;
		mainGame->wLanWindow->setText(dataManager.GetIntraStrings(6));
		if(!on_local)
			ClientEvent(bev,events,ctx);
	}
}
...
int DuelClient::ClientThread(void* param) {
	...
	if(connect_state_s>=3)
	{
		connect_state = 0;
		connect_state_s=2;
			SendPacketToServer(CTOS_STOP_CLIENT);
	}
	if( (int)param <=1 )
	{
		bufferevent_free(client_bev);
...
void DuelClient::HandleSTOCPacketLan(char* data, unsigned int len) {
	...
	//change   
	if(!(connect_state&7) && pktType!=STOC_ERROR_MSG && pktType!=STOC_CHAT && pktType!=STOC_NETWORK_SERVER_ID &&
		pktType!=STOS_VERSION_CHECK)
		return;
	switch(pktType) {
	case STOC_GAME_MSG: {
		if(len - 1 > 0)
	...
	case STOC_JOIN_GAME: {
		...
		mainGame->cbDeckSelect->setEnabled(true);
		mainGame->btnHostAI->setVisible(false);
		if(mainGame->wCreateHost->isVisible())
		{
			mainGame->HideElement(mainGame->wCreateHost);
			if(mainGame->cbServer->getSelected() == 1)
			{
				mainGame->btnHostAI->setVisible(true);
			}
		}
	...
	case STOC_REPLAY: {
		//std::ofstream replay("replay.recv");
		//replay << "test";
		//replay.flush();
		mainGame->gMutex.Lock();
		mainGame->wPhase->setVisible(false);
		...
		time(&nowtime);
		struct tm *当前时间;
		当前时间 = localtime(&nowtime);
		myswprintf(自动存档,L"自动存档：%d年%d月%d日%d点%d分", 1900 + 当前时间->tm_year, 1+ 当前时间->tm_mon, 当前时间->tm_mday,当前时间->tm_hour, 当前时间->tm_min);
		new_replay.SaveReplay(自动存档);
	...
	//new
	case STOC_STOP_CLIENT: {
		/*
		connect_state_s=5;
		mainGame->addWaitingBox(waitfortcp,(void*)&connect_state,dataManager.GetIntraStrings(4));
		*/
		if(is_qre_recording)
			saveQre();
		if(!is_closing) {
	...
	case STOC_NETWORK_SERVER_ID: {
		HostPacket* pkt = (HostPacket*)pdata;
		pkt->ipaddr = SERVER_IP;
		addroomtolist(pkt);	
		break;
	}
	case STOS_VERSION_CHECK: {
		int prover=BufferIO::ReadInt32(pdata);
		int potver=BufferIO::ReadInt32(pdata);
		if(prover!=PRO_VERSION || potver!=POT_VERSION)
		{
			connect_state_s=255;
			event_base_loopbreak(client_base_s);
		}
		break;
	}
	}
...
int DuelClient::ClientAnalyze(char * msg, unsigned int len) {
	...
	case MSG_MOVE: {
		...
		if(reason & 0x1)
			mainGame->PlaySound("./sound/destroyed.wav");
		if (pl == 0) {
	...
	case MSG_CHAINING: {
		...
		CardData data;
		if(dataManager.GetData(code, &data))
		{
			if((data.type & TYPE_SPELL) || (data.type & TYPE_TRAP))
			{
				mainGame->setSW(code);
			}
		}
	...
	case MSG_CHAINED: {
		...
		int code = mainGame->dField.current_chain.code;
		play(code, "c");
	...
	case MSG_ATTACK: {
		...
		play(mainGame->dField.attacker->code, "a");
		return true;
	}
	case MSG_BATTLE: {
		int ca = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
...
void DuelClient::BeginRefreshHost() {
	...
	for(auto it = otherServers.rbegin(); it != otherServers.rend(); ++it)
	{
		mainGame->lstHostList->insertItem(0, it->first.c_str(), -1);
		mainGame->lstHostList->setItemOverrideColor(0, irr::video::SColor(255, 255, 0, 0));
	}
	for(auto it = stickServers.rbegin(); it != stickServers.rend(); ++it)
	{
		mainGame->lstHostList->insertItem(0, it->first.c_str(), -1);
		mainGame->lstHostList->setItemOverrideColor(0, irr::video::SColor(255, 255, 100, 0));
	}
	...
	//change
	timeval timeout = {5, 0};
	resp_event = event_new(broadev, EV_TIMEOUT | reply,EV_READ | EV_PERSIST, BroadcastReply, broadev);
	event_add(resp_event, &timeout);
	Thread::NewThread(RefreshThread, broadev);
	//new
	if(connect_state_s==2)
		SendPacketToServer(CTOS_NETWORK_CLIENT_ID);
	//send request
	SOCKADDR_IN local;
...
//new
void DuelClient::addroomtolist(HostPacket* pHP){
	mainGame->gMutex.Lock();
	std::wstring hoststr;
	hoststr.append(L"[");
	...
	std::wstring name(gamename), onDuel(L"对战中");

	int offset = stickServers.size();

	if(name.length() < 5 || name.substr(1, 3) != onDuel) {
		hosts.push_front(*pHP);
		mainGame->lstHostList->insertItem(offset, hoststr.c_str(), -1);
		mainGame->lstHostList->setItemOverrideColor(offset, 0xff0000ff);
	} else {
		hosts.push_back(*pHP);
		int size = mainGame->lstHostList->getItemCount();
		if(size >= otherServers.size())
		{
			size -= otherServers.size();
		}
		mainGame->lstHostList->insertItem(size, hoststr.c_str(), -1);
		//mainGame->lstHostList->addItem(hoststr.c_str());
	}
	mainGame->gMutex.Unlock();
	if(mainGame->lstHostList->getItemCount() == 1)
	{
		mainGame->lstHostList->setSelected(0);
		irr::SEvent event_call;
		event_call.EventType = irr::EET_GUI_EVENT;
		event_call.GUIEvent.EventType = irr::gui::EGET_LISTBOX_CHANGED;
		event_call.GUIEvent.Caller = ygo::mainGame->lstHostList;
		mainGame->device->postEventFromUser(event_call);
	}
}
}
