	...
	case irr::EET_GUI_EVENT: {
		...
		case irr::gui::EGET_BUTTON_CLICKED: {
			...
			case BUTTON_LEAVE_GAME: {
				...
				if(mainGame->dInfo.player_type == 7) {
					//change
					if(mainGame->actionSignal._waiting || mainGame->replaySignal._waiting || mainGame->singleSignal._waiting)
						break;
			...
			case SCROLL_CARD_SELECT: {
				...
				for(int i = 0; i < 5; ++i) {
					...
					if(selectable_cards[i + pos]->code) {
						...
						mainGame->btnCardSelect[i]->setCover(NULL);
					} else {
						int tagOffset = mainGame->dInfo.tag_player[selectable_cards[i + pos]->controler] ? 2 : 0;
						mainGame->btnCardSelect[i]->setImage(imageManager.¿¨±³Í¼[selectable_cards[i + pos]->controler + tagOffset]);
						mainGame->btnCardSelect[i]->setCover(imageManager.coverTop[selectable_cards[i + pos]->controler + tagOffset]);
					}
			...
			case SCROLL_CARD_DISPLAY: {
				...
				for(int i = 0; i < 5; ++i) {
					...
					if(display_cards[i + pos]->code) {
						...
						mainGame->btnCardDisplay[i]->setCover(NULL);
					} else {
						int tagOffset = mainGame->dInfo.tag_player[display_cards[i + pos]->controler] ? 2 : 0;
						mainGame->btnCardDisplay[i]->setImage(imageManager.¿¨±³Í¼[display_cards[i + pos]->controler + tagOffset]);
						mainGame->btnCardDisplay[i]->setCover(imageManager.coverTop[display_cards[i + pos]->controler + tagOffset]);
					}
		...
		case irr::gui::EGET_ELEMENT_HOVERED: {
			if(id >= BUTTON_CARD_0 && id <= BUTTON_CARD_4) {
				...
				if(mcard->code) {
					int tagOffset = mainGame->dInfo.tag_player[mcard->controler] ? 2 : 0;
					int locationOffset = mcard->code & 0x802040 ? 4 : 0;
					int i = mcard->controler + tagOffset + locationOffset;
					mainGame->imgCover->setImage(imageManager.protector[i] ? imageManager.protector[i] : imageManager.tBlank);
					mainGame->ShowCardInfo(mcard->code);
				} else {
					int tagOffset = mainGame->dInfo.tag_player[mcard->controler] ? 2 : 0;
					if(delay_swap && mcard->controler == swap_player)
						tagOffset = 2 - tagOffset;
					int locationOffset = mcard->location == LOCATION_EXTRA ? 4 : 0;
					mainGame->imgCard->setImage(imageManager.¿¨±³Í¼[mcard->controler + tagOffset + locationOffset]);
					mainGame->imgCover->setImage(imageManager.coverTop[mcard->controler + tagOffset + locationOffset]);
	...
	case irr::EET_MOUSE_INPUT_EVENT: {
		...
		case irr::EMIE_MOUSE_MOVED: {
			...
			if(!panel || !panel->isVisible() || !panel->getRelativePosition().isPointInside(mousepos)) {
				...
				} else if(hovered_location == LOCATION_EXTRA && extra[hovered_controler].size()) {
					mcard = *(extra[hovered_controler].rbegin());
				} else {
					mcard = 0;
				}
			}
...
void ClientField::ShowMenu(int flag, int x, int y) {
	...
	mainGame->btnCardCopy->setVisible(false);
	mainGame->btnDeckReturn->setVisible(false);
	mainGame->btnCardClear->setVisible(false);
	mainGame->btnCardCopyToSide->setVisible(false);
	mainGame->btnCardMoveToSide->setVisible(false);
	//
	int height = 1;
