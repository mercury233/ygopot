...
const unsigned short POT_VERSION = 0x2;
//new
//const unsigned int SERVER_IP=inet_addr("139.196.166.78");
//const unsigned int SERVER_IP=inet_addr("103.253.96.167");
const unsigned int SERVER_IP=inet_addr("112.124.124.227");
//const unsigned int SERVER_IP = inet_addr("127.0.0.1");
const unsigned short SERVER_PORT=8911;

int Thread::进程数=0;
bool Thread::退出进程=false;
...
int WaitingBoxThread(void* texv) {
	IGUIStaticText* tex = (IGUIStaticText*) texv;
	int n=0;
	wchar_t oldtex[28];
	myswprintf( oldtex,L"%ls",tex->getText() );
	wchar_t newtex[28];
	while(true)
	{
		myswprintf( newtex,L"%ls",oldtex );
		int i=-1;
		while(++i<n)
		{
			myswprintf( newtex,L"%ls.",newtex );
		}
		myswprintf( newtex,L"%ls>_<",newtex );
		tex->setText(newtex);
		if(++n>4)n=0;
		Sleep(700);
		if(wbcheckfun(wbcheckvar)){
			tex->getParent()->remove();
			return 0;
		}
	}
}
...
Game* mainGame;
//new
irr::io::IFileSystem* DataManager::fsys;

void replace(std::wstring &str, std::wstring &pat, std::wstring &sub)
{
	std::size_t index = str.find(pat);
	if(index == std::wstring::npos)
		return;
	str.replace(index, pat.length(), sub);
}

void Game::generateTempSkin()
{
	std::ifstream xmlFile("skin\\pat.xml");
	if(!xmlFile.is_open())
		return;
	...
	std::wstring files[15] = {L"button", L"ButtonPressed", L"ButtonDisabled", L"MenuBar", L"MenuPane",
		L"MenuPressed", L"CheckBox", L"sunkenpane", L"CheckBoxDisabled", L"TabControl", L"TabButton",
		L"TabButtonPressed", L"windowbg2", L"gauge1", L"gauge2"
	};

	for(int i = 0; i < 15; i++) {
		std::wstring dir(L"skin\\");
		std::wstring	pat(files[i] + L".png");
		dir.append(files[i].begin(), files[i].end());
		std::wstring name = DataManager::randomGet(dir.c_str(), L"png");
		if(!name.empty())
			replace(wcontents, pat, L"../" + name);
	}
	std::wstring_convert<std::codecvt_utf8<wchar_t>> conv;
	contents = conv.to_bytes(wcontents);
	std::ofstream tmpXmlFile("skin\\skin.xml");
...
class NullLogger : public ILogger {
	...
	virtual ELOG_LEVEL getLogLevel() const {return ELL_INFORMATION;}
};

bool Game::Initialize() {
	srand(time(0));

	DuelClient::stickServers.push_back(std::make_pair(L"[测试服务器]新萌卡对战平台", L"run://新萌卡对战平台.exe"));
	DuelClient::stickServers.push_back(std::make_pair(L"[稳定服务器]游侠对战平台", L"run://游侠对战平台\\游侠对战平台.exe"));
	DuelClient::stickServers.push_back(std::make_pair(L"[深夜服务器]外国服务器checkmate", L"ip://173.224.211.158:21001"));
	DuelClient::stickServers.push_back(std::make_pair(L"[广告位招租]游戏王贴吧", L"http://tieba.baidu.com/f?ie=utf-8&kw=%E6%B8%B8%E6%88%8F%E7%8E%8B"));
	DuelClient::stickServers.push_back(std::make_pair(L"[广告位招租]游戏王ygocore贴吧", L"http://tieba.baidu.com/f?ie=utf-8&kw=%E6%B8%B8%E6%88%8F%E7%8E%8Bygocore"));
	//DuelClient::stickServers.push_back(std::make_pair(L"[测试服务器]菜菜对战平台", L"run://菜菜对战平台.exe"));
	...
	irr::os::Printer::Logger = new NullLogger();
	int nScreenWidth, nScreenHeight;
	nScreenWidth = GetSystemMetrics(SM_CXSCREEN);
	nScreenHeight = GetSystemMetrics(SM_CYSCREEN);
	double size=(nScreenHeight-128)/640.0;
	params.WindowSize = irr::core::dimension2d<u32>(1024*size, 640*size);
	device = irr::createDeviceEx(params);
	if(!device)
	{
		params.DriverType = params.DriverType==irr::video::EDT_DIRECT3D9 ?irr::video::EDT_OPENGL:irr::video::EDT_DIRECT3D9;
		device = irr::createDeviceEx(params);
		if(!device)
			return false;
	}
	...
	//new
	DataManager::fsys = device->getFileSystem();
	std::vector<std::wstring> filelist = DataManager::listDir(L"expansions",L"*.zip");
	char filename[256];
	int listcount = filelist.size();
	while(--listcount>=0)
	{
		DataManager::fsys->addFileArchive(filelist[listcount].c_str(), false, false, EFAT_ZIP, "QQ1783658607");
	}
	//new
	std::vector<std::wstring> dirlist= DataManager::listDir(L"expansions",L"*",true);
	int dircount = dirlist.size();
	while(--dircount>=0){
		filelist = DataManager::listDir(dirlist[dircount].c_str(),L"*.cdb");
		listcount = filelist.size();
		while(--listcount>=0){
			BufferIO::EncodeUTF8(filelist[listcount].c_str(), filename);
			dataManager.LoadDB(filename);
		}
	}
	filelist = DataManager::listDir(L"expansions",L"*.cdb");
	listcount = filelist.size();
	while(--listcount>=0)
	{
		BufferIO::EncodeUTF8(filelist[listcount].c_str(), filename);
	...
	textFont = guiFont;
	//new
	textFont->setKerningWidth(1);
	textFont->setKerningHeight(1);
	...
	//main menu
	wchar_t strbuf[256];
	myswprintf(strbuf, L"YGOPot %X.0%X.%X V1", PRO_VERSION >> 12, (PRO_VERSION >> 4) & 0xff, PRO_VERSION & 0xf);
	//change

	video::SColor white(255, 255, 255, 224);

	wm =  new CGUIWindowManager(env, env->getRootGUIElement(), device->getCursorControl());
	core::array<CGUIAdvancedWindow::buttoninfo> ButtonInfo;
	wMainMenu = wm->addWindow(rect<s32>(370, 180, 650, 495), strbuf, ButtonInfo, irr::gui::EWC_HIDE, imageManager.tWindBG);
	//wMainMenu = env->addWindow(rect<s32>(370, 180, 650, 460), false, strbuf);
	//wMainMenu->getCloseButton()->setVisible(false);
	btnLanMode = env->addButton(rect<s32>(10, 30, 270, 60), wMainMenu, BUTTON_LAN_MODE, dataManager.GetSysString(1200));
	dynamic_cast<CGUIButton*>(btnLanMode)->setOverrideColor(white);

	btnServerMode = env->addButton(rect<s32>(10, 65, 270, 95), wMainMenu, BUTTON_SINGLE_MODE, dataManager.GetSysString(1201));
	dynamic_cast<CGUIButton*>(btnServerMode)->setOverrideColor(white);

	btnReplayMode = env->addButton(rect<s32>(10, 100, 270, 130), wMainMenu, BUTTON_REPLAY_MODE, dataManager.GetSysString(1202));
	dynamic_cast<CGUIButton*>(btnReplayMode)->setOverrideColor(white);
//	btnTestMode = env->addButton(rect<s32>(10, 135, 270, 165), wMainMenu, BUTTON_TEST_MODE, dataManager.GetSysString(1203));
	...
	dynamic_cast<CGUIButton*>(btnDeckEdit)->setOverrideColor(white);

	btnHelpMode = env->addButton(rect<s32>(10, 240, 270, 270), wMainMenu, BUTTON_HELP_MODE, L"文字帮助");
	dynamic_cast<CGUIButton*>(btnHelpMode)->setOverrideColor(white);
	//temp
	btnModeExit = env->addButton(rect<s32>(10, 275, 270, 305), wMainMenu, BUTTON_MODE_EXIT, dataManager.GetSysString(1210));
	dynamic_cast<CGUIButton*>(btnModeExit)->setOverrideColor(white);

	卡组下载按钮 = env->addButton(rect<s32>(10, 170, 270, 200), wMainMenu, 卡组下载, L"卡组下载");
	dynamic_cast<CGUIButton*>(卡组下载按钮)->setOverrideColor(white);

	btnWebSites = env->addButton(rect<s32>(10, 205, 270, 235), wMainMenu, BUTTON_WEBSITES, L"网页帮助");
	dynamic_cast<CGUIButton*>(btnWebSites)->setOverrideColor(white);
	//
	//lan mode
	//change
	wLanWindow = env->addWindow(rect<s32>(220, 100, 800, 520), false, dataManager.GetIntraStrings(6));
	...
	if(gameConf.lflist >= 0 && gameConf.lflist < cbLFlist->getItemCount())
		cbLFlist->setSelected(gameConf.lflist);
	...
	if(gameConf.rule >= 0 && gameConf.rule < cbRule->getItemCount())
		cbRule->setSelected(gameConf.rule);
	...
	if(gameConf.pkmode >= 0 && gameConf.pkmode < cbMatchMode->getItemCount())
		cbMatchMode->setSelected(gameConf.pkmode);

	//new
	env->addStaticText(dataManager.GetSysString(2041), rect<s32>(20, 120, 220, 140), false, false, wCreateHost);
	cbServer = env->addComboBox(rect<s32>(140, 115, 300, 135), wCreateHost);
	cbServer->addItem(dataManager.GetIntraStrings(1));
	cbServer->addItem(dataManager.GetIntraStrings(2));

	if(gameConf.server >= 0 && gameConf.server < cbServer->getItemCount())
		cbServer->setSelected(gameConf.server);

	//change
	env->addStaticText(dataManager.GetSysString(1237), rect<s32>(20, 150, 320, 170), false, false, wCreateHost);
	myswprintf(strbuf, L"%d", gameConf.gameTimer);
	ebTimeLimit = env->addEditBox(strbuf, rect<s32>(140, 145, 220, 170), true, wCreateHost);
	ebTimeLimit->setTextAlignment(irr::gui::EGUIA_CENTER, irr::gui::EGUIA_CENTER);

	env->addStaticText(dataManager.GetSysString(1228), rect<s32>(20, 180, 320, 200), false, false, wCreateHost);
	chkEnablePriority = env->addCheckBox(false, rect<s32>(20, 210, 170, 230), wCreateHost, -1, dataManager.GetSysString(1236));

	chkEnablePriority->setChecked(gameConf.enablePriority);

	//change
	//chkEnablePriority->setVisible(0);
	chkNoCheckDeck = env->addCheckBox(false, rect<s32>(20, 240, 170, 260), wCreateHost, -1, dataManager.GetSysString(1229));

	chkNoCheckDeck->setChecked(gameConf.noCheckDeck);

	chkNoShuffleDeck = env->addCheckBox(false, rect<s32>(180, 240, 360, 260), wCreateHost, -1, dataManager.GetSysString(1230));

	chkNoShuffleDeck->setChecked(gameConf.noShuffleDeck);

	env->addStaticText(dataManager.GetSysString(1231), rect<s32>(20, 270, 320, 290), false, false, wCreateHost);
	myswprintf(strbuf, L"%d", gameConf.startLP);
	ebStartLP = env->addEditBox(strbuf, rect<s32>(140, 265, 220, 290), true, wCreateHost);
	ebStartLP->setTextAlignment(irr::gui::EGUIA_CENTER, irr::gui::EGUIA_CENTER);
	env->addStaticText(dataManager.GetSysString(1232), rect<s32>(20, 300, 320, 320), false, false, wCreateHost);
	myswprintf(strbuf, L"%d", gameConf.startHand);
	ebStartHand = env->addEditBox(strbuf, rect<s32>(140, 295, 220, 320), true, wCreateHost);
	ebStartHand->setTextAlignment(irr::gui::EGUIA_CENTER, irr::gui::EGUIA_CENTER);
	env->addStaticText(dataManager.GetSysString(1233), rect<s32>(20, 330, 320, 350), false, false, wCreateHost);
	myswprintf(strbuf, L"%d", gameConf.DrawCount);
	ebDrawCount = env->addEditBox(strbuf, rect<s32>(140, 325, 220, 350), true, wCreateHost);
	...
	btnHostAI = env->addButton(rect<s32>(130, 180, 230, 205), wHostPrepare, BUTTON_HOST_AI, L"添加AI");
	...
	//img
	wCardImg = env->addStaticText(L"", rect<s32>(1, 1, 244, 336), true, false, 0, -1, true);
	wCardImg->setBackgroundColor(0xc0c0c0c0);
	wCardImg->setVisible(false);
	imgCard = env->addImage(rect<s32>(0, 0, 243, 335), wCardImg);
	imgCover = env->addImage(rect<s32>(0, 0, 243, 335), wCardImg);
	imgCard->setUseAlphaChannel(true);
	imgCard->setScaleImage(true);
	imgCover->setUseAlphaChannel(true);
	imgCover->setScaleImage(true);

	imgCard->setImage(imageManager.卡背图[0]);
	imgCover->setImage(imageManager.tBlank);
	...
	//tab //change
	wInfos = env->addTabControl(rect<s32>(1, 340, 301, 639), 0, true);
	...
	stText = env->addStaticText(L"", rect<s32>(15, 83, 287, 299), false, true, tabInfo, -1, false);
	scrCardText = env->addScrollBar(false, rect<s32>(267, 83, 287, 299), tabInfo, SCROLL_CARDTEXT);
	...
	chkAutoPos = env->addCheckBox(false, rect<s32>(20, 5, 280, 30), tabSystem, -1, dataManager.GetSysString(1274));
	chkAutoPos->setChecked(true);
	chkRandomPos = env->addCheckBox(false, rect<s32>(40, 35, 300, 60), tabSystem, -1, dataManager.GetSysString(1275));
	chkAutoChain = env->addCheckBox(false, rect<s32>(20, 65, 280, 90), tabSystem, -1, dataManager.GetSysString(1276));
	chkWaitChain = env->addCheckBox(false, rect<s32>(20, 95, 280, 120), tabSystem, -1, dataManager.GetSysString(1277));
	chkIgnore1 = env->addCheckBox(false, rect<s32>(20, 155, 280, 180), tabSystem, -1, dataManager.GetSysString(1290));
	chkIgnore2 = env->addCheckBox(false, rect<s32>(20, 185, 280, 210), tabSystem, -1, dataManager.GetSysString(1291));
	chkIgnore2->setChecked(false);
	//new sound
	chkEnableSound = env->addCheckBox(gameConf.enablesound, rect<s32>(20, 215, 100, 240), tabSystem, CHECKBOX_ENABLE_SOUND, dataManager.GetSysString(2046));
	scrSound = env->addScrollBar(true, rect<s32>(100, 220, 280, 235), tabSystem, SCROLL_SOUND);
	scrSound->setMax(100);
	scrSound->setMin(0);
	scrSound->setPos(gameConf.soundvolume * 100);
	scrSound->setLargeStep(1);
	scrSound->setSmallStep(1);
	chkEnableMusic  = env->addCheckBox(gameConf.enablemusic, rect<s32>(20, 240, 100, 265), tabSystem, CHECKBOX_ENABLE_MUSIC, dataManager.GetSysString(2047));
	scrMusic = env->addScrollBar(true, rect<s32>(100, 245, 280, 260), tabSystem, SCROLL_MUSIC);
	scrMusic->setMax(100);
	scrMusic->setMin(0);
	...
	auto stFontSize = env->addStaticText(L"字体大小:", rect<s32>(20, 125, 95, 150), false, false, tabSystem);
	scrFont = env->addScrollBar(true, rect<s32>(100, 130, 280, 140), tabSystem, SCROLL_FONT);
	scrFont->setMax(18);
	scrFont->setMin(10);
	scrFont->setPos(gameConf.textfontsize);
	scrFont->setLargeStep(1);
	scrFont->setSmallStep(1);

	//
	//new
	wFloat = env->addWindow(rect<s32>(0, 0, 200, 90), false, L"");
	wFloat->getCloseButton()->setVisible(false);
	wFloat->setDrawTitlebar(false);
	wFloat->setDraggable(false);
	wFloat->setVisible(false);
	floatFont = irr::gui::CGUITTFont::createTTFont(env, gameConf.textfont,15);
	floatFont->setKerningWidth(2);
	floatFont->setKerningHeight(2);
	stFloat=env->addStaticText(L"", rect<s32>(10, 0, 190, 90), false, true, wFloat);
	stFloat->setOverrideFont(floatFont);
	stFloat->setOverrideColor(video::SColor(255,220,30,30));
	//
	...
	btnPSAD->setImage(imageManager.卡背图[0], rect<s32>(0, 0, 177, 254));
	...
	btnPSDD->setImage(imageManager.卡背图[0], rect<s32>(0, 0, 177, 254));
	//card select
	...
	//change deck edit
	btnCardClear = env->addButton(rect<s32>(1, 1, 99, 21), wCmdMenu, BUTTON_CMD_CardClear, L"清除");
	btnCardCopy = env->addButton(rect<s32>(1, 1, 99, 21), wCmdMenu, BUTTON_CMD_CardCopy, L"复制");
	btnDeckReturn = env->addButton(rect<s32>(1, 1, 99, 21), wCmdMenu, BUTTON_CMD_DeckReturn, L"撤销");
	btnCardCopyToSide = env->addButton(rect<s32>(1, 1, 99, 21), wCmdMenu, BUTTON_CMD_CardCopyToSide, L"复制到Side");
	btnCardMoveToSide = env->addButton(rect<s32>(1, 1, 99, 21), wCmdMenu, BUTTON_CMD_CardMoveToSide, L"移动到Side");
	...
	cbDBLFList->setMaxSelectionRows(20);
	...
	cbDBDecks->setMaxSelectionRows(20);//
	for(unsigned int i = 0; i < deckManager._lfList.size(); ++i)
	...
	cbCardType->setMaxSelectionRows(20);
	...
	cbCardType2 = env->addComboBox(rect<s32>(130, 3, 190, 23), wFilter, COMBOBOX_CardType2);
	...
	cbLimit = env->addComboBox(rect<s32>(260, 3, 390, 23), wFilter, COMBOBOX_Limit);
	cbLimit->setMaxSelectionRows(20);
	...
	cbAttribute = env->addComboBox(rect<s32>(60, 26, 190, 46), wFilter, COMBOBOX_Attribute);
	...
	cbRace = env->addComboBox(rect<s32>(60, 49, 190, 69), wFilter, COMBOBOX_Race);
	cbRace->setMaxSelectionRows(20);
	...
	ebAttack = env->addEditBox(L"", rect<s32>(260, 26, 340, 46), true, wFilter,EDITBOX_Attack);
	ebAttack->setTextAlignment(irr::gui::EGUIA_CENTER, irr::gui::EGUIA_CENTER);
	stLabel8 = env->addStaticText(dataManager.GetSysString(1323), rect<s32>(205, 51, 280, 71), false, false, wFilter);
	ebDefence = env->addEditBox(L"", rect<s32>(260, 49, 340, 69), true, wFilter,EDITBOX_Defence);
	...
	ebStar = env->addEditBox(L"", rect<s32>(60, 72, 140, 92), true, wFilter,EDITBOX_Star);
	ebStar->setTextAlignment(irr::gui::EGUIA_CENTER, irr::gui::EGUIA_CENTER);

	stLabel12 = env->addStaticText(L"字段：", rect<s32>(10, 97, 80, 117), false, false, wFilter);
	cbSetcode = env->addComboBox(rect<s32>(60, 97, 190, 117), wFilter, COMBOBOX_Setcode);
	cbSetcode->addItem(dataManager.GetSysString(1310), 0);
	cbSetcode->setMaxSelectionRows(20);
	for(auto it = dataManager.setcodes.begin(); it != dataManager.setcodes.end(); ++it)
	{
		cbSetcode->addItem(it->second.c_str(), it->first);
	}
	...
	btnQreConvert = env->addButton(rect<s32>(360, 355, 440, 380), wReplay, BUTTON_QRE_CONVERT, L"yrp转qre");
	...
	//replay control
	wReplayControl = env->addStaticText(L"", rect<s32>(250, 143, 340, 273), true, false, 0, -1, true);
	...
	//help mod
	wHelpWindow = env->addWindow(rect<s32>(220, 100, 800, 520), false, L"文字帮助");
	wHelpWindow->getCloseButton()->setVisible(false);
	wHelpWindow->setVisible(false);
	lstHelpList = env->addListBox(rect<s32>(10, 30, 180, 365), wHelpWindow, LISTBOX_HELP_LIST, true);
	lstHelpList->setItemHeight(18);
	helpFont = irr::gui::CGUITTFont::createTTFont(env, gameConf.textfont,16);
	helpFont->setKerningWidth(2);
	helpFont->setKerningHeight(2);
	ebHelp = env->addEditBox(L"", rect<s32>(195, 30, 565, 400), true, wHelpWindow);
	ebHelp->setMultiLine(true);
	ebHelp->setDrawBorder(false);
	ebHelp->setOverrideFont(helpFont);
	ebHelp->setOverrideColor(video::SColor(255,200,20,30));
	ebHelp->setTextAlignment(irr::gui::EGUIA_UPPERLEFT,irr::gui::EGUIA_UPPERLEFT);
	btnHelpCancel = env->addButton(rect<s32>(30, 375, 125, 400), wHelpWindow, BUTTON_HELP_EXIT, dataManager.GetSysString(1347));

	//web mod
	wWebWindow = env->addWindow(rect<s32>(220, 100, 800, 520), false, L"网页帮助");
	wWebWindow->getCloseButton()->setVisible(false);
	wWebWindow->setVisible(false);
	lstCataList = env->addListBox(rect<s32>(10, 30, 180, 365), wWebWindow, LISTBOX_CATA_LIST, true);
	lstCataList->setItemHeight(18);
	lstWebList = env->addListBox(rect<s32>(195, 30, 565, 400), wWebWindow, LISTBOX_WEB_LIST, true);
	lstWebList->setItemHeight(18);
	btnWebCancel = env->addButton(rect<s32>(30, 375, 125, 400), wWebWindow, BUTTON_WEB_CANCEL, dataManager.GetSysString(1347));

	//change
	btnLeaveGame = env->addButton(rect<s32>(205, 143, 295, 273), 0, BUTTON_LEAVE_GAME, L"");
	...
	for (u32 i = 0; i < EGDC_COUNT; ++i) {
		...
		//env->getSkin()->setColor((EGUI_DEFAULT_COLOR)i, SColor(255, 255, 255, 224));
	}
	//new
...
void Game::MainLoop() {
	...
	while(device->run()) {
		...
		if(dInfo.isStarted) {
			if(!dueling) {
				imageManager.setBackGround(DataManager::randomGet(L"textures\\bg2", L"jpg"));
				dueling = true;
			}
			
			//add
			if(mainGame->showcardcode == 1 || mainGame->showcardcode == 3)
				Game::PlayMusic2(L"sound\\duelwin",L"mp3",true);
			else if(mainGame->showcardcode == 2)
				Game::PlayMusic2(L"sound\\duellose",L"mp3",true);
			else if(mainGame->dInfo.lp[0] == 0)
				Game::PlayMusic2(L"sound\\rockpaperscissors",L"mp3",true);
			else if(mainGame->dInfo.lp[LocalPlayer(0)] < mainGame->dInfo.lp[LocalPlayer(1)]/2)
				Game::PlayMusic2(L"sound\\song-disadvantage",L"mp3",true);
			else if(mainGame->dInfo.lp[LocalPlayer(0)] > mainGame->dInfo.lp[LocalPlayer(1)]*2)
				Game::PlayMusic2(L"sound\\song-advantage",L"mp3",true);
			else
				Game::PlayMusic2(L"sound\\song",L"mp3",true);
			DrawBackGround();
			DrawCards();
			DrawSW();
			DrawMisc();
			...
			DrawDeckBd();
			Game::PlayMusic2(L"sound\\deck",L"mp3",true);
		}else {
			if(dueling) {
				imageManager.setBackGround(DataManager::randomGet(L"textures\\bg", L"jpg"));
				dueling = false;
			}
			Game::PlayMusic2(L"sound\\menu",L"mp3",true);
		}
		...
		if(cur_time >= 1000) {
			...
			if (++tran > 5)
			{
				...
				//floatFont->setTransparency(true);
				tran = 0;
			}
...
#endif
	SaveConfig();
	int i=DataManager::fsys->getFileArchiveCount();
	while(--i>=0)
	{
		DataManager::fsys->removeFileArchive(i);
	}
	delete cmovies;
	device->drop();
	if(engineMusic)
...
void Game::SetStaticText(irr::gui::IGUIElement* pControl, u32 cWidth, irr::gui::CGUITTFont* font, const wchar_t* text, u32 pos) {
	int pbuffer = 0;
	//change
	wchar_t *line=dataManager.strBuffer,*head=dataManager.strBuffer;
	u32 _height = 0;
	for(int i = 0; text[i] != 0 && i < 1023; ++i) {
		dataManager.strBuffer[pbuffer] = text[i];
		dataManager.strBuffer[pbuffer+1] = 0;
		core::dimension2d<u32> dim = font->getDimension(line);
		if(dim.Width>cWidth && dataManager.strBuffer[pbuffer] != L'\n')
		{
			dataManager.strBuffer[pbuffer+1] = dataManager.strBuffer[pbuffer];
			dataManager.strBuffer[pbuffer] = L'\n';
			line=dataManager.strBuffer+pbuffer+1;
			if(++_height == pos)
				head = line;
			pbuffer++;
...
void Game::RefreshReplay() {
	...
	fh = FindFirstFileW(L"./qre/*.qre", &fdataw);
	if(fh == INVALID_HANDLE_VALUE)
		return;
	do 
	{
		...
		{
			lstReplayList->addItem(fdataw.cFileName);
			lstReplayList->setItemOverrideColor(lstReplayList->getItemCount() - 1, video::SColor(255, 0, 0, 255));
		}
	} while(FindNextFileW(fh, &fdataw));
...
void Game::LoadConfig() {
	...
	gameConf.pkmode = 0;
	gameConf.server = 0;
	gameConf.gameTimer = 180;
	gameConf.enablePriority = 0;
	gameConf.noCheckDeck = 0;
	gameConf.noShuffleDeck = 0;
	gameConf.startLP = 8000;
	gameConf.startHand = 5;
	gameConf.DrawCount = 1;
	...
	while(ftell(fp) < fsize) {
		...
		//new sound
		} else if(!strcmp(strbuf, "lflist")) {
			gameConf.lflist = atoi(valbuf);
		} else if(!strcmp(strbuf, "rule")) {
			gameConf.rule = atoi(valbuf);
		} else if(!strcmp(strbuf, "pkmode")) {
			gameConf.pkmode = atoi(valbuf);
		} else if(!strcmp(strbuf, "server")) {
			gameConf.server = atoi(valbuf);
		} else if(!strcmp(strbuf, "gameTimer")) {
			gameConf.gameTimer = atoi(valbuf);
		} else if(!strcmp(strbuf, "enablePriority")) {
			gameConf.enablePriority = atoi(valbuf);
		} else if(!strcmp(strbuf, "noCheckDeck")) {
			gameConf.noCheckDeck = atoi(valbuf);
		} else if(!strcmp(strbuf, "noShuffleDeck")) {
			gameConf.noShuffleDeck = atoi(valbuf);
		} else if(!strcmp(strbuf, "startLP")) {
			gameConf.startLP = atoi(valbuf);
		} else if(!strcmp(strbuf, "starthand")) {
			gameConf.startHand = atoi(valbuf);
		} else if(!strcmp(strbuf, "drawCount")) {
			gameConf.DrawCount = atoi(valbuf);
			...
			if (!strcmp(strbuf, "nickname")) {
				...
				sprintf(gameConf.other[gameConf.othernum++],"%s",linebuf);
			}
		}
	}
	fclose(fp);
...
void Game::SaveConfig() {
	...
	fprintf(fp, "pkmode = %d\n", gameConf.pkmode);
	fprintf(fp, "server = %d\n", gameConf.server);
	fprintf(fp, "gameTimer = %d\n", gameConf.gameTimer);
	fprintf(fp, "enablePriority = %d\n", gameConf.enablePriority);
	fprintf(fp, "noCheckDeck = %d\n", gameConf.noCheckDeck);
	fprintf(fp, "noShuffleDeck = %d\n", gameConf.noShuffleDeck);
	fprintf(fp, "startLP = %d\n", gameConf.startLP);
	fprintf(fp, "starthand = %d\n", gameConf.startHand);
	fprintf(fp, "drawCount = %d\n", gameConf.DrawCount);
	//add
	fprintf(fp, "enable_music = %d\n", gameConf.enablemusic?1:0);
	fprintf(fp, "music_volume = %d\n", int(gameConf.musicvolume*100));
	fprintf(fp, "enable_sound = %d\n", gameConf.enablesound?1:0);
	fprintf(fp, "sound_volume = %d\n", int(gameConf.soundvolume*100));
	int num=-1;
	while(++num < gameConf.othernum)
	{
		fprintf(fp, "%s", gameConf.other[num]);
	}
	fclose(fp);
}
//change
void Game::ShowCardInfo(int code) {
	...
	if(cd.type & TYPE_MONSTER) {
		...
		formatBuffer[0] = L'[';
		myswprintf(formatBuffer, L"[★%d] ",cd.level);
		if(cd.attack < 0 && cd.defence < 0)
			myswprintf(&formatBuffer[5], L"?/?");
		else if(cd.attack < 0)
			myswprintf(&formatBuffer[5], L"?/%d", cd.defence);
		else if(cd.defence < 0)
			myswprintf(&formatBuffer[5], L"%d/?", cd.attack);
		else
			myswprintf(&formatBuffer[5], L"%d/%d", cd.attack, cd.defence);
		...
		stText->setRelativePosition(recti(15, 83, 296 * window_size.Width / 1024 - 30, 284 * window_size.Height / 640));
		scrCardText->setRelativePosition(recti(stInfo->getRelativePosition().getWidth() - 20, 83, stInfo->getRelativePosition().getWidth(), 264 * window_size.Height / 640));
	} else {
		myswprintf(formatBuffer, L"[%ls]", dataManager.FormatType(cd.type));
		stInfo->setText(formatBuffer);
		stDataInfo->setText(L"");
		stText->setRelativePosition(recti(15, 60, 296 * window_size.Width / 1024 - 30, 284 * window_size.Height / 640));
		scrCardText->setRelativePosition(recti(stInfo->getRelativePosition().getWidth() - 20, 83, stInfo->getRelativePosition().getWidth(), 264 * window_size.Height / 640));
	}
...
//new
irr::gui::IGUIWindow* Game::addWaitingBox(int (*checkfun)(void*),void* checkvar,const wchar_t* text,
		bool modal,s32 id)
{
	core::rect<s32> rect2;
	core::dimension2d<u32> screenDim, msgBoxDim;
	screenDim.Width = params.WindowSize.Width;
	screenDim.Height = params.WindowSize.Height;
	msgBoxDim.Width = 256*1024/params.WindowSize.Width;
	msgBoxDim.Height = 96*640/params.WindowSize.Height;
	rect2.UpperLeftCorner.X = (screenDim.Width - msgBoxDim.Width) / 2;
	rect2.UpperLeftCorner.Y = (screenDim.Height - msgBoxDim.Height) / 2;
	rect2.LowerRightCorner.X = rect2.UpperLeftCorner.X + msgBoxDim.Width;
	rect2.LowerRightCorner.Y = rect2.UpperLeftCorner.Y + msgBoxDim.Height;
	IGUIWindow* win = env->addWindow(rect2,modal,L"",0,id);
	win->getCloseButton()->setVisible(false);
	IGUIStaticText* tex=env->addStaticText(text, rect<s32>(15*1024/params.WindowSize.Width,45*640/params.WindowSize.Height, msgBoxDim.Width,msgBoxDim.Height), false, false, win);
	
	wbcheckfun=checkfun;
	wbcheckvar=checkvar;
	Thread::NewThread(WaitingBoxThread,(void*)tex);
	return win;
}
...
void Game::OnResize()
{
	wMainMenu->setRelativePosition(ResizeWin(370, 180, 650, 495));
	wWebWindow->setRelativePosition(ResizeWin(220, 100, 800, 520));
	...
	wCardImg->setRelativePosition(Resize(1, 1, 244, 336));
	imgCard->setRelativePosition(Resize(0, 0, 243, 335));
	imgCover->setRelativePosition(Resize(0, 0, 243, 335));
	//change
	wInfos->setRelativePosition(Resize(1, 340, 301, 639));
	...
	stText->setRelativePosition(recti(15, stText->getRelativePosition().UpperLeftCorner.X, 296 * window_size.Width / 1024 - 30, 284 * window_size.Height / 640));
	scrCardText->setRelativePosition(recti(stInfo->getRelativePosition().getWidth() - 20, 83, stInfo->getRelativePosition().getWidth(), 264 * window_size.Height / 640));
	lstLog->setRelativePosition(Resize(10, 10, 290, 290));
	btnClearLog->setRelativePosition(Resize(160, 300, 260, 325));

	btnLeaveGame->setRelativePosition(Resize(245, 5, 325, 70));
	wReplayControl->setRelativePosition(Resize(250, 143, 340, 273));
	...
	wHelpWindow->setRelativePosition(ResizeWin(220, 100, 800, 520));
	stLabel12->setRelativePosition(ResizeWin(10, 97, 80, 117));
	cbSetcode->setRelativePosition(Resize(60, 97, 190, 117));
}
...
void Game::PlayMusic2(wchar_t* dir,wchar_t* shuffix, bool loop){
	static char song[256];
	static wchar_t* name=L"";
	if(gameConf.enablemusic && engineMusic){
		
		if(!engineMusic->isCurrentlyPlaying(song) || name != dir){
					BufferIO::EncodeUTF8(DataManager::randomGet( dir, shuffix).c_str(),song);
					name = dir;
					engineMusic->stopAllSounds();
					engineMusic->play2D(song, false);
...
void Game::PlaySound(char* sound){
	if(gameConf.enablesound && engineSound){
