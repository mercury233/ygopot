...
public:
	...
	explicit 随机数(unsigned int seed) :
		left(1) {
		init(seed);
	}
	随机数(unsigned int* init_key, int key_length) :
		left(1) {
		int i = 1, j = 0;
	...
	int 取正整数(int num1,int num2=0)
	{
		int ret= num1>=num2 ? num2+real()*(num1-num2) : num1+real()*(num2-num1);
		return ret>0 ?ret :0;
	}
private:
	void next_state() {
